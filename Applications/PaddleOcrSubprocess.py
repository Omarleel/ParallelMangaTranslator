...
class PaddleOcrSubprocess:
    ...
    def _start(self) -> None:
        ...
        env = os.environ.copy()
        env["PMT_PADDLE_LANG"] = self.lang
        env["PMT_OCR_GPU"] = "1" if self.use_gpu else "0"
        env.setdefault("PADDLE_PDX_DISABLE_MODEL_SOURCE_CHECK", "1")
        env.setdefault("FLAGS_allocator_strategy", "auto_growth")

        cmd = [sys.executable, "-m", "Applications.PaddleOcrWorker", "--lang", self.lang, "--gpu", env["PMT_OCR_GPU"]]
