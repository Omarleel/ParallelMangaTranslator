...
class BubbleTextRulesMixin:
    ...
    def _looks_like_sfx(self, detections: Sequence) -> bool:
        text = "".join(self.geometry.text(det) for det in detections).strip()
        if self.onomatopoeia_manager.is_onomatopoeia_candidate(text, self.idioma_entrada):
            return True
        boxes = [self.geometry.to_rect(det) for det in detections]
        if not boxes:
            return False
        merged = boxes[0]
        for box in boxes[1:]:
    ...
    def _should_keep_free_text_group(
        ...
        cjk_vertical_ocr_retry = (
            ...
            and area_ratio <= min(self.free_text.hard_max_area_ratio, 0.060)
            and symbol_ratio <= 0.55
        )

        # Nada que parezca texto y además baja confianza: probablemente ruido.
        if not has_signal and not looks_sfx and confidence < self.free_text.min_confidence:
        ...
        if not looks_sfx:
            ...
            if weak_short_signal and confidence < max(0.35, self.free_text.min_confidence):
        ...
        if area_ratio > self.free_text.hard_max_area_ratio:
        ...
        is_large = (
            area_ratio > self.free_text.max_area_ratio
    ...
    def _detection_group_merge_decision(self, a: Box, b: Box) -> Dict[str, object]:
        ...
        merge = False
        reason = "separados"
        max_vertical_gap = max(8.0, avg_h * self.ocr_merge.y_gap_ratio)
        max_cjk_horizontal_gap = max(6.0, avg_w * self.ocr_merge.cjk_x_gap_ratio)
        ...
        max_line_gap = max(6.0, avg_w * self.ocr_merge.line_x_gap_ratio)
        ...
        if x_overlap >= self.ocr_merge.x_overlap and gap_y <= max_vertical_gap:
            merge = True
            reason = "fragmentos_verticales_mismo_bloque"
        elif (
            self.ocr_merge.cjk_columns
            and self.idioma_entrada in {"Japonés", "Chino", "Coreano"}
            and y_overlap >= self.ocr_merge.cjk_y_overlap
            and gap_x <= max_cjk_horizontal_gap
        ):
            merge = True
            reason = "columnas_cjk_mismo_bloque"
        elif (
            (not self.ocr_merge.line_horizontal_only or both_horizontalish)
            and y_overlap >= self.ocr_merge.line_y_overlap
        ...
        return {
            ...
            "thresholds": {
                "x_overlap": self.ocr_merge.x_overlap,
                "vertical_gap": round(float(max_vertical_gap), 3),
                "cjk_y_overlap": self.ocr_merge.cjk_y_overlap,
                "cjk_horizontal_gap": round(float(max_cjk_horizontal_gap), 3),
                "line_y_overlap": self.ocr_merge.line_y_overlap,
                "line_gap": round(float(max_line_gap), 3),
                "cjk_columns_enabled": self.ocr_merge.cjk_columns,
                "line_horizontal_only": self.ocr_merge.line_horizontal_only,
            },
            "orientation": {
    ...
    def _group_detections(self, detections: Sequence, trace_decisions: Optional[List[Dict[str, object]]] = None) -> List[List]:
        ...
        for det_idx, det in enumerate(detections):
            try:
                rect = self.geometry.to_rect(det)
            except Exception:
                continue
            placed = False
            for i, (box, items) in enumerate(groups):
                ...
                if trace_decisions is not None and len(trace_decisions) < self.merge_debug_pair_limit:
                    ...
                    trace_item.update({
                        ...
                        "detection_text": self.geometry.text(det),
                    })
                    trace_decisions.append(trace_item)
                if decision["merge"]:
                    groups[i] = (self.geometry.union(box, rect), items + [det])
                    placed = True
                    break
            if not placed:
            ...
            for i, (box, items) in enumerate(groups):
                ...
                for j in range(i + 1, len(groups)):
                    ...
                    if trace_decisions is not None and len(trace_decisions) < self.merge_debug_pair_limit:
                        ...
                        trace_decisions.append(trace_item)
                    if decision["merge"]:
                        current_box = self.geometry.union(current_box, other_box)
