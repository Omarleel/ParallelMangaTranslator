"""Reparto del texto de un globo entre varias ranuras (globos partidos, lóbulos).

Era un mixin, y lo único que le ataba a serlo es que llamaba a `_normalize_text`, que vivía
en otro mixin hermano. Esa función ahora es `text_prep.normalize_text`.
"""
...
def sentence_units(texto: str) -> List[str]:
...
def split_text_for_slots(texto: str, slot_count: int) -> List[str]:
    slot_count = max(1, int(slot_count))
    texto = normalize_text(texto)
    if slot_count == 1:
        return [texto]

    units = sentence_units(texto)
...
__all__ = ["sentence_units", "split_text_for_slots"]
