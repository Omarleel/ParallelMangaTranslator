"""Reparto de un globo en lóbulos, para repartir el texto entre ellos.

Era un mixin de `TextRenderer` y sus siete funciones son puras: lo único que tomaba del
anfitrión eran dos funciones de máscara que ahora viven en `mask_text_area`.
"""
...
from parallel_manga_translator.rendering.mask_text_area import (
    normalize_clip_mask,
    safe_text_area_from_mask,
)


def component_records(binary_mask: np.ndarray, min_area: int) -> List[Tuple[np.ndarray, Tuple[int, int, int, int], int]]:
...
def component_centroid(component: np.ndarray, fallback_box: Tuple[int, int, int, int]) -> Tuple[float, float]:
...
def lobe_slots_from_centers(
    ...
    *,
    ...
    for idx in range(len(centers)):
        ...
        slot = safe_text_area_from_mask(lobe_inner, width, height, style)
        sx, sy, sw, sh = slot
    ...
    if len(slots) < 2:
        return []
    return order_slots_for_reading(slots, right_to_left=right_to_left)


def distance_lobe_slots_from_mask(
    mask: np.ndarray,
    width: int,
    height: int,
    style: str,
    *,
    ...
    for frac in (0.72, 0.68, 0.64, 0.60, 0.56, 0.52):
        ...
        components = component_records(seed, min_area=max(20, int(total_area * 0.0025)))
        if len(components) < 2:
            continue
        ...
        centers = [component_centroid(comp, box) for comp, box, _area in filtered]
    ...
    return lobe_slots_from_centers(
        mask,
        best_centers,
        width,
        height,
...
def connected_lobe_slots_from_mask(
    ...
    *,
    ...
    mask = normalize_clip_mask(local_mask, width, height)
    total_area = int(cv2.countNonZero(mask))
    ...
    for ratio in (0.055, 0.075, 0.095, 0.12, 0.15):
        ...
        components = component_records(eroded, min_area=min_area)
        if len(components) < 2:
            continue
    ...
    if len(best_components) < 2:
        return distance_lobe_slots_from_mask(
            mask,
            width,
            height,
            style,
    ...
    centers = [box_center(box) for _comp, box, _area in best_components]
    slots = lobe_slots_from_centers(
        mask,
        centers,
        width,
        height,
...
__all__ = ["box_center", "component_centroid", "component_records", "connected_lobe_slots_from_mask", "distance_lobe_slots_from_mask", "lobe_slots_from_centers", "order_slots_for_reading"]
