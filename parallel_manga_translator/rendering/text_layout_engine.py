"""Dónde y a qué tamaño va cada texto. La mitad de `TextRenderer` que no pinta.

`TextRenderer` eran 823 líneas propias que hacían dos cosas: **medir** —decidir la caja, el
tamaño de fuente, el corte de líneas, los lóbulos de un globo partido— y **pintar**. La
dirección del acoplamiento lo delataba: `render` llama a `build_layout` y
`render_with_layouts` a `resolve_manual_layout`, nunca al revés.

Aquí vive la mitad de medida, con las métricas de fuente y el ajuste de texto que necesita.
`TextRenderer` conserva ambas como fachada, porque `build_layout` y `resolve_manual_layout`
son API de la UI: el editor guarda su salida en `Layout UI` y la reutiliza al recomponer.

Nada de esto lo vigila `eval_runner`, que no rotula. Lo vigila `test_text_layout_golden.py`.
"""
...
from parallel_manga_translator.rendering.slot_text_splitter import split_text_for_slots
from parallel_manga_translator.rendering.text_fitting_mixin import TextFittingMixin


class TextLayoutEngine(FontMetricsMixin, TextFittingMixin):
    """Calcula la distribución del texto. No toca píxeles."""
    ...
    def resolve_manual_layout(
        ...
        *,
        ...
        if isinstance(raw_blocks, list) and raw_blocks:
            ...
            block_texts = split_text_for_slots(prepared_text, len(raw_blocks)) if len(raw_blocks) > 1 else [prepared_text]
    ...
    def build_layout(
        ...
        *,
        ...
        split_slots = connected_lobe_slots_from_mask(
            clip_mask,
            w,
            h,
            style,
        ...
        if split_slots:
            slot_texts = split_text_for_slots(texto, len(split_slots))
            render_blocks = list(zip(split_slots, slot_texts))
        else:
            safe_x, safe_y, safe_w, safe_h = safe_text_area_from_mask(clip_mask, w, h, style)
    ...
    def _scale_local_box(raw_box: Any, src_w: int, src_h: int, dst_w: int, dst_h: int) -> Tuple[int, int, int, int]:
        x, y, w, h = TextLayoutEngine._layout_box_values(raw_box, (0, 0, dst_w, dst_h))
