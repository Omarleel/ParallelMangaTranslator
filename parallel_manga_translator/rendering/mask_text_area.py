"""Áreas seguras de escritura dentro de la máscara de un globo.

Era un mixin de `TextRenderer`, pero sus tres funciones son puras: no leían un solo atributo
de su anfitrión. Como módulo se pueden usar —y probar— sin construir el renderizador entero,
que carga fuentes.
"""
...
def normalize_clip_mask(clip_mask: np.ndarray, width: int, height: int) -> np.ndarray:
...
def largest_rect_from_row_spans(binary_mask: np.ndarray) -> Tuple[int, int, int, int]:
    """
...
def safe_text_area_from_mask(local_mask: Optional[np.ndarray], width: int, height: int, style: str) -> Tuple[int, int, int, int]:
    """
    ...
    sx, sy, sw, sh = largest_rect_from_row_spans(inner)
...
__all__ = ["largest_rect_from_row_spans", "normalize_clip_mask", "safe_text_area_from_mask"]
